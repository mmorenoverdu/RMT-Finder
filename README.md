...
2. Absolute Search Space: Use the full range of the stimulation device as the search space (from 1% to 100% MSO), or an arbitrary wide range (e.g. 20% to 80% MSO). This will be slower as the algorithm will test intensities implausibly lower/higher for a given individual (e.g. if the hotspot is found with intensities around 40% MSO, it is not necessary to test the RMT at 70% MSO). However, this Absolute and the Relative methods will converge to the same RMT within ± 2% MSO according to our previous data ([Boidequin et al. 2026]). We do not recommend the Absolute method because it is slower and high intensities might cause unnecesary discomfort for the person. Therefore we highly recommend the Relative method.
